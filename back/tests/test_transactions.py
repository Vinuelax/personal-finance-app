...
def test_create_get_update_delete_transaction(client: TestClient):
    ...
    # get
    resp = client.get(f"/transactions/{txn_id}", params={"date": payload["date"]})
    assert resp.status_code == 200
    # update
    payload_update = payload | {"merchant": "Updated Merchant"}
    resp = client.patch(f"/transactions/{txn_id}", params={"date": payload["date"]}, json=payload_update)
    assert resp.status_code == 200
    assert resp.json()["merchant"] == "Updated Merchant"

    # delete
    resp = client.delete(f"/transactions/{txn_id}", params={"date": payload["date"]})
    assert resp.status_code == 200
...
def test_transactions_filters(client: TestClient):
    # date range
    resp = client.get("/transactions", params={"date_from": "2025-11-15", "date_to": "2025-11-16"})
    assert resp.status_code == 200
    data = resp.json()
    for item in data:
    ...
    # uncategorized only
    resp = client.get("/transactions", params={"uncategorized": "true"})
    assert resp.status_code == 200
    data = resp.json()
    for item in data:
